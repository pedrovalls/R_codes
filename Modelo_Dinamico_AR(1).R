...
model_ar1 <- lm(log(cons) ~ log(cons1), data = datagive1[2:159,])

# Summary of the model to display regression results
summary(model_ar1)

# Using White correction
coeftest(model_ar1, vcov = vcovHC(model, type = "HC0"))
#using Newey-West Correction
coeftest(model_ar1, vcov = vcovHAC(model))

num = length(datagive1$cons)-1
num
AIC_ar1= AIC(model_ar1)
AIC_ar1
AIC_ar1_T=AIC_ar1/num
AIC_ar1_T
BIC_ar1 = BIC(model_ar1)

BIC_ar1
BIC_ar1_T=BIC_ar1/num
BIC_ar1_T
npar_ar1 = 2
loglik_ar1 = -(1/2)*(AIC_ar1 - 2*npar_ar1)
loglik_ar1

# Create a data frame of predicted values and standard errors
predictions_ar1 <- data.frame(
  lcons_ar1 = log(datagive1$cons[2:159]),
  Predicted_ar1 = predict(model_ar1, interval = "prediction")[,1],
  Lower_ar1 = predict(model_ar1, interval = "prediction")[,2],
  Upper_ar1 = predict(model_ar1, interval = "prediction")[,3]
)
...
ggplot(predictions_ar1, aes(x = datagive1$Date[2:159])) +
  geom_line(aes(y = Predicted_ar1, color = "Predicted"), size = 1) +  # Define color within aes for legend
  geom_ribbon(aes(ymin = Lower_ar1, ymax = Upper_ar1, fill = "Confidence Interval"), alpha = 0.2) +  # Define fill within aes for legend
  geom_point(aes(y = lcons_ar1, color = "Observed"), size = 2) +  # Define color within aes for legend
...
sigma_value_ar1 = sigma(model_ar1)
sigma_value_ar1

ggplot(predictions_ar1, aes(x = datagive1$Date[2:159], y = Residuals_ar1)) +
  ...
  geom_hline(aes(yintercept = 2 * sigma_value_ar1, linetype = "2*sigma"), color = "blue") +  # Map linetype to aes
  geom_hline(aes(yintercept = -2 * sigma_value_ar1, linetype = "-2*sigma"), color = "gray") +  # Map linetype to aes
