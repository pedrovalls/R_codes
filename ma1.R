...
plot(yma, type='l', col='blue', main = 'MA(1) with theta = 0.5')
yma_ts = ts(yma)

# define series lagged one period for the time series gwn call it yma_1
yma_1=lag(yma_ts, k=-1)
# define series lagged two period for the time series gwn call it yma_2
yma_2=lag(yma_ts, k=-2)
#define series lagged twenty periods for the time series gwn call it yma_20
yma_20=lag(yma_ts, k=-20)
