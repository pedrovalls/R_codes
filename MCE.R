...
##
# Correlation between CONS INC INFLAT
##
new_data = cbind.data.frame(datagive1$cons,datagive1$cons1,datagive1$inc,datagive1$inc1,datagive1$inflat,datagive1$inflat1)
new_data = new_data[2:159,]
cor_new_data = cor(new_data,method=c("spearman"))
cor_new_data
...
model_ADL<- lm(log(cons) ~ log(cons1)+log(inc)+log(inc1), data = datagive1[2:159,])
...
coeftest(model_ADL, vcov = vcovHC(model_ADL, type = "HC0"))
#using Newey-West Correction
coeftest(model_ADL, vcov = vcovHAC(model_ADL))

num = length(datagive1$cons)-1
num
AIC_ADL= AIC(model_ADL)
AIC_ADL
AIC_ADL_T=AIC_ADL/num
AIC_ADL_T
BIC_ADL = BIC(model_ADL)

BIC_ADL
BIC_ADL_T=BIC_ADL/num
BIC_ADL_T
npar_ADL = length(model_ADL$coefficients)
loglik_ADL = -(1/2)*(AIC_ADL - 2*npar_ADL)
loglik_ADL

# Create a data frame of predicted values and standard errors
predictions_ADL <- data.frame(
  lcons_ADL = log(datagive1$cons[2:159]),
  Predicted_ADL = predict(model_ADL, interval = "prediction")[,1],
  Lower_ADL = predict(model_ADL, interval = "prediction")[,2],
  Upper_ADL = predict(model_ADL, interval = "prediction")[,3]
)

# Save  residuals 
Residuals_ADL <- residuals(model_ADL)

# Plot observed vs predicted values with confidence intervals



ggplot(predictions_ADL, aes(x = datagive1$Date[2:159])) +
  geom_line(aes(y = Predicted_ADL, color = "Predicted"), size = 1) +  # Define color within aes for legend
  geom_ribbon(aes(ymin = Lower_ADL, ymax = Upper_ADL, fill = "Confidence Interval"), alpha = 0.2) +  # Define fill within aes for legend
  geom_point(aes(y = lcons_ADL, color = "Observed"), size = 2) +  # Define color within aes for legend
...
sigma_value_ADL = sigma(model_ADL)
sigma_value_ADL

ggplot(predictions_ADL, aes(x = datagive1$Date[2:159], y = Residuals_ADL)) +
  ...
  geom_hline(aes(yintercept = 2 * sigma_value_ADL, linetype = "2*sigma"), color = "blue") +  # Map linetype to aes
  geom_hline(aes(yintercept = -2 * sigma_value_ADL, linetype = "-2*sigma"), color = "gray") +  # Map linetype to aes
...
###
# Test log run elasticity and Homogeneity
##


test_log_run_elasticity_chi <-linearHypothesis(model_ADL,c("log(cons1) = 1.0","log(inc) = -log(inc1)"), test ="Chisq")
test_log_run_elasticity_F <-linearHypothesis(model_ADL,c("log(cons1) = 1.0","log(inc) = -log(inc1)"), test ="F")
test_log_run_elasticity_chi
test_log_run_elasticity_F
...
model_ADL_Inflat<- lm(log(cons) ~ log(cons1)+log(inc)+log(inc1)+inflat+inflat1, data = datagive1[2:159,])
...
test_log_run_elasticity_chi <-linearHypothesis(model_ADL_Inflat,c("inflat = 0.0","inflat1 = 0"), test ="Chisq")
test_log_run_elasticity_F <-linearHypothesis(model_ADL_Inflat,c("inflat = 0.0","inflat1 = 0"), test ="F")
test_log_run_elasticity_chi
test_log_run_elasticity_F
